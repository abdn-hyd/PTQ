...
class ModelArgs:
    ...
    vocab_size: int = -1
    # multipler for hidden_dim in FFN
    ...
    # for hf adaptation and params.json
    hf: bool = False
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    end: int, the max seq length
    theta: float, rope frequency
    """
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        # MODIFIED: Assume a single machine, so world size is 1.
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.feed_forward = FeedForward(
            dim=args.dim,
            hidden_dim=4 * args.dim if not args.hf else args.intermediate_size,
